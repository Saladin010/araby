-- Script to assign StudentNumber to existing students who don't have one
-- Run this in SQL Server Management Studio or using dotnet ef database execute
...
SELECT Id
FROM AspNetUsers
WHERE Role = 1 AND StudentNumber IS NULL  -- Role 1 = Student
ORDER BY CreatedAt;
...
-- Verify the results
SELECT COUNT(*) AS 'Students with StudentNumber'
FROM AspNetUsers
WHERE Role = 1 AND StudentNumber IS NOT NULL;

SELECT COUNT(*) AS 'Students without StudentNumber'
FROM AspNetUsers
